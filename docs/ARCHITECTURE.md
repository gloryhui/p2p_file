...
- 候选地址进签名：中间人无法在「客户端 → 服务器」这一段改写登记者的候选地址。
  **安全边界**：服务器转发给对端的 `PeerCandidates` 与打洞令牌**没有**再签名、也没有
  加密，所以路径上的主动攻击者仍能篡改对端收到的候选/令牌，制造打洞或连接失败
  （DoS）。它无法借此冒充业务对端——那由 QUIC 之上的应用层握手（§4.7）确认。
...
服务器内存里维护 `peers`（在线节点 → 候选地址 + 发送队列 + 连接所有权凭证）和
`waiters`（哪条**连接**在等哪个节点上线）。牵线时机有两个：对方登记时唤醒等待者、
以及自己主动查询时。两处都可能触发，所以同一对节点可能被牵线两次——这也是令牌
一致性测试存在的原因。
...
- **连接所有权凭证（`connection_id`）。** 每个 accept 分配一个单调递增 ID 记进
  `PeerRecord`。断开时只有 `record.connection_id` 与自己的相同时才删记录；被取代的
  旧连接不能再 `Lookup`，而且服务器在登记新连接时**主动 `close` 掉旧连接**——否则
  拿同一个私钥不停重连就能让旧连接靠 `Ping` 无限续命，socket/task 无限堆积。
- **waiter 也带所有权。** `waiters[target]` 存的是 `(requester_node_id, connection_id)`，
  不是裸的 `NodeId`。这样旧连接的清理不会删掉新连接创建的等待，旧连接留下的等待也不会
  被新连接「继承」——唤醒时校验 `connection_id` 仍是该 requester 的当前连接。
- **配额看真实 pending。** 单连接的挂起查询数由 Registry 的
  `pending_by_connection` 统计，目标上线、`try_pair` 消费掉 waiter 时同步删除，
  配额随之释放；不会把「已经完成的历史目标」永久算进配额。
...
而且持有期间**必须持续心跳**：`SignalingClient` 后台每
`DEFAULT_HEARTBEAT_INTERVAL`（默认 60s < `idle_timeout` 180s）自动发一次 `Ping`，
否则隧道长时间活跃时上层根本不碰信令，服务器到点就会把这个「在线」节点摘掉。
后台 reader 会一直把 `Pong` 读掉并丢弃，避免长时间不读把 TCP 缓冲撑满、反过来把
服务器的 writer 卡死。
...
9. 信令服务器是单点，且重启后在线表清空（客户端会重新登记，但需要一次重试）。
   登记已有私钥挑战认证与资源上限（见 §4.5），但**信令链路是明文 TCP，且保护范围
   不覆盖转发段**：注册签名只保证「客户端 → 服务器」这一段登记内容的身份与完整性；
   服务器转发给对端的候选地址/打洞令牌没有端到端签名或加密，路径上的主动攻击者可以
   篡改它们制造 DoS（不能冒充业务对端）。服务器仍是可用性单点。
