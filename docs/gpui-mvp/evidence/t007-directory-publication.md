...
| `cargo test --locked --offline --features gui --lib` | 376 PASS，0 failed/ignored |
| `cargo test --locked --offline --features gui --lib desktop::` | 106 PASS，0 failed/ignored |
...
- 根以下 symlink 和原路径替换链接不被跟随；staging .part/.bitmap 链接不触碰未选择文件；NFC/大小写别名（含 σ/ς、I/ı、straße/STRASSE）明确拒绝，扩展断言先复现再修复。最大合法 basename 使用固定 staging 内部名成功发布并清理 data.part；新增清理断言先复现失败，再修复通过。回执重发会重试此前清理。
- Unix 已有 FIFO 无对端也立即拒绝；回归先复现阻塞，再以 no-follow + O_NONBLOCK 修复，失败测试会主动释放旧阻塞线程而不挂住测试进程；Linux/macOS 均使用原生 mkfifo 创建真实 fixture，未跳过 macOS 断言。
