...
6项独立边界测试覆盖：实际格式架构不允许混平台/IntelMac；实际PE import RVA解析与越界拒绝；危险路径拒绝；dirty预检不能accepted及归档损坏被检测；Mach-O deployment只取minos/legacyOS version，忽略SDK/linker tool version并仍拒绝真实14.0/13.0.1。PowerShell host选择/子环境模块路径隔离/Unicode路径与missing-host拒绝。完整 verify 实际解包、逐文件和794份源码哈希检查。最终包SHA还需与CI artifact中的candidate.json对照。
...
第二候选 `6d9fc7f6aaa7b7a901152c6d109ebe2f2be1b28c` 的macOS/Linux完整GUI、包与校验通过；Windows416和优化编译/metadata通过，但签名检查的PowerShell5继承Core模块搜索路径导致Security模块加载失败。修复隔离PSModulePath、优先pwsh并保留PS5默认模块路径fallback；缺少host或非NotSigned仍拒绝，不伪造签名状态。新增该环境边界回归。旧head三平台集不合格，不用于新候选门禁；原生600与实际LinuxCI包30秒两方向证据各自保留原SHA。
